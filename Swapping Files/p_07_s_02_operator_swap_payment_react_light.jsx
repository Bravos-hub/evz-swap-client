import React, { useEffect, useState } from "react";

const evzOperatorSwapPayStyles = `
:root {
  --evz-primary: #03cd8c;
  --evz-accent: #f77f00;
  --evz-bg: #f5f7fb;
  --evz-surface: #ffffff;
  --evz-text-primary: #111827;
  --evz-text-secondary: #6b7280;
  --evz-border-subtle: rgba(15, 23, 42, 0.08);
}

body {
  margin: 0;
  padding: 0;
  background: var(--evz-bg);
  font-family: system-ui, -apple-system, BlinkMacSystemFont, "Inter",
    "SF Pro Text", sans-serif;
}

.evz-app {
  min-height: 100vh;
  background: var(--evz-bg);
  display: flex;
  justify-content: center;
  align-items: stretch;
}

.evz-screen {
  width: 100%;
  max-width: 430px;
  min-height: 100vh;
  padding: 24px 20px 20px;
  box-sizing: border-box;
  display: flex;
  flex-direction: column;
}

.evz-header {
  padding-bottom: 8px;
}

.evz-header-title {
  font-size: 20px;
  font-weight: 800;
  margin: 0;
}

.evz-header-subtitle {
  font-size: 13px;
  color: var(--evz-text-secondary);
  margin: 4px 0 0;
}

.evz-content {
  flex: 1;
  padding-top: 16px;
  padding-bottom: 10px;
}

.evz-section-title {
  font-size: 13px;
  font-weight: 700;
  margin: 0 0 8px;
}

.evz-payment-list {
  display: flex;
  flex-direction: column;
  gap: 10px;
}

.evz-pay-option {
  display: flex;
  align-items: center;
  gap: 12px;
  background-color: var(--evz-surface);
  border-radius: 16px;
  padding: 10px 12px;
  border: 1.5px solid transparent;
  box-shadow: 0 8px 18px rgba(15, 23, 42, 0.04);
  cursor: pointer;
  transition: border-color 0.16s ease, box-shadow 0.16s ease,
    background-color 0.16s ease;
}

.evz-pay-option--selected {
  border-color: var(--evz-primary);
  box-shadow: 0 10px 30px rgba(3, 205, 140, 0.2);
  background-color: #f9fffd;
}

.evz-pay-radio {
  width: 20px;
  height: 20px;
  border-radius: 999px;
  border: 1.5px solid #d1d5db;
  display: flex;
  align-items: center;
  justify-content: center;
  flex-shrink: 0;
  background-color: #ffffff;
}

.evz-pay-radio-dot {
  width: 10px;
  height: 10px;
  border-radius: 999px;
  background: var(--evz-accent);
}

.evz-pay-icon {
  width: 28px;
  height: 28px;
  border-radius: 999px;
  background: #e5f5ff;
  display: flex;
  align-items: center;
  justify-content: center;
  font-size: 14px;
}

.evz-pay-icon--evz {
  background: rgba(3, 205, 140, 0.08);
  color: var(--evz-primary);
}

.evz-pay-label-main {
  font-size: 14px;
  font-weight: 600;
}

.evz-pay-label-sub {
  font-size: 11px;
  color: var(--evz-text-secondary);
}

.evz-pay-text {
  display: flex;
  flex-direction: column;
}

.evz-summary-card {
  margin-top: 16px;
  padding: 14px 16px;
  border-radius: 16px;
  background: #eef2ff;
  font-size: 13px;
  display: flex;
  flex-direction: column;
}

.evz-summary-label {
  color: var(--evz-text-secondary);
}

.evz-summary-value {
  font-weight: 700;
}

.evz-summary-heading {
  font-weight: 700;
  color: var(--evz-text-primary);
  margin-bottom: 2px;
}

.evz-summary-sub {
  color: var(--evz-text-secondary);
  margin-bottom: 8px;
}

.evz-summary-row {
  display: flex;
  justify-content: space-between;
  margin-top: 4px;
}

.evz-summary-row-label {
  color: var(--evz-text-secondary);
}

.evz-summary-row-amount {
  font-weight: 600;
}

.evz-summary-total {
  display: flex;
  justify-content: space-between;
  margin-top: 10px;
  font-weight: 800;
  color: var(--evz-text-primary);
}

.evz-footer {
  padding-top: 10px;
}

.evz-primary-cta {
  width: 100%;
  border: none;
  border-radius: 999px;
  padding: 12px 16px;
  background: var(--evz-accent);
  color: #ffffff;
  font-size: 15px;
  font-weight: 600;
  cursor: pointer;
  box-shadow: 0 12px 30px rgba(247, 127, 0, 0.35);
  display: flex;
  align-items: center;
  justify-content: center;
  gap: 6px;
}

.evz-primary-cta:disabled {
  opacity: 0.5;
  box-shadow: none;
  cursor: default;
}

.evz-secondary-cta {
  margin-top: 8px;
  width: 100%;
  border: none;
  background: transparent;
  color: var(--evz-text-secondary);
  font-size: 13px;
  font-weight: 500;
  text-decoration: underline;
  cursor: pointer;
}
`;

function useEvzOperatorSwapPayStyles() {
  useEffect(() => {
    if (typeof document === "undefined") return;
    if (document.getElementById("evz-operator-swap-pay-styles")) return;
    const style = document.createElement("style");
    style.id = "evz-operator-swap-pay-styles";
    style.innerHTML = evzOperatorSwapPayStyles;
    document.head.appendChild(style);
  }, []);
}

function EvzScreen({ children }: { children: React.ReactNode }) {
  return (
    <div className="evz-app">
      <div className="evz-screen">{children}</div>
    </div>
  );
}

type PaymentMethodId = "evz" | "card" | "mobile" | "alipay" | "wechat";

const PAYMENT_METHODS: {
  id: PaymentMethodId;
  label: string;
  sub?: string;
  iconType: "evz" | "card" | "wallet" | "globe" | "bolt";
}[] = [
  { id: "evz", label: "EVzone Pay", sub: "Recommended • Low fees", iconType: "bolt" },
  { id: "card", label: "Card", sub: "Visa, Mastercard", iconType: "card" },
  {
    id: "mobile",
    label: "Mobile Money",
    sub: "MTN, Airtel & more",
    iconType: "wallet",
  },
  { id: "alipay", label: "Alipay", iconType: "globe" },
  { id: "wechat", label: "WeChat Pay", iconType: "globe" },
];

function fmtUGX(n: number) {
  return `UGX ${Number(n || 0).toLocaleString("en-UG")}`;
}

interface OperatorSwapPaymentProps {
  /** Optional override for total amount due. If omitted, it is computed
   *  from energyKWh * ratePerKWh + platformFee + processingFee
   */
  amountDue?: number;
  energyKWh?: number;
  ratePerKWh?: number;
  platformFee?: number;
  processingFee?: number;
}

const P07S02OperatorSwapPayment: React.FC<OperatorSwapPaymentProps> = ({
  amountDue,
  energyKWh = 2.4,
  ratePerKWh = 3500,
  platformFee = 300,
  processingFee = 50,
}) => {
  useEvzOperatorSwapPayStyles();

  const [selectedId, setSelectedId] = useState<PaymentMethodId>("evz");

  // FIX: use the correctly cased ratePerKWh variable
  const energyFee = energyKWh * ratePerKWh;
  const computedTotal = energyFee + platformFee + processingFee;
  const total =
    typeof amountDue === "number" && amountDue > 0 ? amountDue : computedTotal;

  const handleConfirm = () => {
    // hook into your real payment flow here
    if (typeof window !== "undefined") {
      window.alert(
        `Paying ${fmtUGX(total)} via ${
          PAYMENT_METHODS.find((m) => m.id === selectedId)?.label
        }`
      );
    }
  };

  return (
    <EvzScreen>
      <header className="evz-header">
        <h1 className="evz-header-title">Operator swap payment</h1>
        <p className="evz-header-subtitle">
          Confirm how you’d like to pay the operator.
        </p>
      </header>

      <main className="evz-content">
        <section>
          <p className="evz-section-title">Choose a payment method</p>
          <div className="evz-payment-list">
            {PAYMENT_METHODS.map((m) => {
              const selected = m.id === selectedId;
              return (
                <button
                  key={m.id}
                  type="button"
                  className={
                    "evz-pay-option" +
                    (selected ? " evz-pay-option--selected" : "")
                  }
                  onClick={() => setSelectedId(m.id)}
                >
                  <div className="evz-pay-radio">
                    {selected && <div className="evz-pay-radio-dot" />}
                  </div>

                  <div
                    className={
                      "evz-pay-icon" +
                      (m.id === "evz" ? " evz-pay-icon--evz" : "")
                    }
                  >
                    {m.iconType === "bolt" && "⚡"}
                    {m.iconType === "card" && "💳"}
                    {m.iconType === "wallet" && "📱"}
                    {m.iconType === "globe" && "🌐"}
                  </div>

                  <div className="evz-pay-text">
                    <span className="evz-pay-label-main">{m.label}</span>
                    {m.sub && (
                      <span className="evz-pay-label-sub">{m.sub}</span>
                    )}
                  </div>
                </button>
              );
            })}
          </div>
        </section>

        <section className="evz-summary-card">
          <div className="evz-summary-heading">Payment summary</div>
          <div className="evz-summary-sub">
            Review the swap fees before confirming payment.
          </div>

          <div className="evz-summary-row">
            <span className="evz-summary-row-label">
              Energy ({energyKWh.toFixed(2)} kWh × UGX {ratePerKWh.toLocaleString(
                "en-UG"
              )}
              /kWh)
            </span>
            <span className="evz-summary-row-amount">{fmtUGX(energyFee)}</span>
          </div>

          <div className="evz-summary-row">
            <span className="evz-summary-row-label">Platform fee</span>
            <span className="evz-summary-row-amount">
              {fmtUGX(platformFee)}
            </span>
          </div>

          <div className="evz-summary-row">
            <span className="evz-summary-row-label">Processing fee</span>
            <span className="evz-summary-row-amount">
              {fmtUGX(processingFee)}
            </span>
          </div>

          <div className="evz-summary-total">
            <span>Total</span>
            <span>{fmtUGX(total)}</span>
          </div>

          <div className="evz-summary-label" style={{ marginTop: 8 }}>
            Billed via {PAYMENT_METHODS.find((m) => m.id === selectedId)?.label}
          </div>
        </section>
      </main>

      <footer className="evz-footer">
        <button
          type="button"
          className="evz-primary-cta"
          disabled={!total}
          onClick={handleConfirm}
        >
          Confirm &amp; pay {fmtUGX(total)}
        </button>
        <button
          type="button"
          className="evz-secondary-cta"
          onClick={() => {
            if (typeof window !== "undefined") window.history.back();
          }}
        >
          Cancel
        </button>
      </footer>
    </EvzScreen>
  );
};

export default P07S02OperatorSwapPayment;

/**
 * Manual test cases:
 * 1) Default props (no overrides):
 *    - Energy 2.40 kWh × UGX 3,500 → UGX 8,400
 *      Platform fee 300, Processing 50 → Total UGX 8,750.
 *    - Payment summary rows match these values and CTA shows "Confirm & pay UGX 8,750".
 * 2) Pass amountDue={15000} → total row & CTA show UGX 15,000 (override breakdown sum).
 * 3) amountDue=0 but custom fees → total computed from breakdown and CTA enabled.
 * 4) Tapping different methods updates "Billed via ..." text.
 * 5) Mobile viewport 360x780: no horizontal scroll; list & summary fill width.
 * 6) Smoke test for bugfix: energyFee uses ratePerKWh (correct casing) and no
 *    ReferenceError is thrown when rendering.
 */
