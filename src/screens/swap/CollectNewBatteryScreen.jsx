...
function useEvzStyles() {
  useEffect(() => {
    ...
    if (document.getElementById("evz-mobile-styles-p10-s03")) return;
    const style = document.createElement("style");
    style.id = "evz-mobile-styles-p10-s03";
...
export default function CollectNewBatteryScreen({
  nextPath = "/swap/self/scan-new",
}) {
  useEvzStyles();
  ...
  const handleConfirm = (e) => {
    ...
    try {
      if (typeof window !== "undefined") {
        window.localStorage.setItem(SELF_NS + "newBatteryId", val);
      }
    } catch {
      // ignore
    }
  ...
  const handleBack = () => {
    if (typeof window !== "undefined") {
      window.location.assign("/swap/self/insert-return");
    }
  };
  ...
  return (
    <EvzScreen>
      <header className="evz-header">
        <h1 className="evz-header-title">Collect your battery</h1>
        <p className="evz-header-subtitle">
          Take the charged pack from slot <strong>{lockerId}</strong>, then scan
          to confirm.
        </p>
      </header>
