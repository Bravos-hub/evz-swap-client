...
function useEvzStyles() {
  useEffect(() => {
    ...
    if (document.getElementById("evz-mobile-styles-p26-s01")) return;
    const style = document.createElement("style");
    style.id = "evz-mobile-styles-p26-s01";
...
export default function LockerTimeoutScreen({ retryHref = "/swap/self/locker" }) {
  useEvzStyles();
  ...
  const expiredAt = useMemo(
    () => Number(getSelf("locker.unlockAt", "0")) || 0,
    []
  );

  const elapsedMs = Math.max(0, now - expiredAt);

  const handleReunlock = () => {
  ...
  return (
    <EvzScreen>
      <header className="evz-header">
        <h1 className="evz-header-title">Locker timed out</h1>
        <p className="evz-header-subtitle">
          Your locker closed before it was opened
        </p>
      </header>
      ...
      <main className="evz-main">
        <section className="evz-card-warning">
          <div className="evz-card-title">Locker</div>
          <div className="evz-locker-rows">
            <div className="evz-locker-col">
              <div className="evz-locker-label">ID</div>
              <div className="evz-locker-value">{lockerId}</div>
            </div>
            <div className="evz-locker-col">
              <div className="evz-locker-label">Time since expiry</div>
              <div className="evz-locker-value">{msToClock(elapsedMs)}</div>
            </div>
          </div>
          <p className="evz-caption">
            We can re-unlock your assigned locker for another 60 seconds.
          </p>
        </section>
      </main>
