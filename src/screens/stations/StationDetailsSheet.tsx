...
function getStationIdFromLocation(): void {
...
export default function StationDetailsSheet(): JSX.Element {
  useEvzStyles();
  ...
  const handleContinue = (): void => {
    if (!station) return;
    navigate(`${ROUTES.HOLD_SELECT}?stationId=${encodeURIComponent(station.id)}`);
  };
        ...
        {!station && (
          <p className="evz-notes-text">
            ...
            <button
              ...
              style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0, textDecoration: 'underline', color: 'inherit' }}
            >
              Go back
            </button>
            .
